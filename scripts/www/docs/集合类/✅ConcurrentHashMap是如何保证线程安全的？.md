...
[✅ConcurrentHashMap为什么在JDK 1.8中废弃分段锁？](docs/集合类/✅ConcurrentHashMap为什么在JDK 1.8中废弃分段锁？.md)
...
<font style="color:rgb(64, 64, 64);">当要插入的桶是空的时候，使用 CAS (</font>`**<font style="color:rgb(64, 64, 64);background-color:rgb(236, 236, 236);">casTabAt</font>**`<font style="color:rgb(64, 64, 64);">) 来尝试将新节点放入该桶，而不是直接加锁。如果 CAS 失败，说明发生了竞争，则转而使用加锁的策略。</font>

<font style="color:rgb(64, 64, 64);"></font>

<font style="color:rgb(64, 64, 64);">当要操作的桶不为空时（可能是链表或红黑树），会对该桶的头节点进行 </font>`**<font style="color:rgb(64, 64, 64);background-color:rgb(236, 236, 236);">synchronized</font>**`<font style="color:rgb(64, 64, 64);"> 加锁。这样，一个线程在操作链表 a 时，另一个线程仍然可以同时操作链表 b，并发度大大提升。</font>
